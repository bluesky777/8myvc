...
| `Autoriza::esAdministrativo` exige un rol `Secretario` que no existe en la base | [05 §26.1](05-codigo-muerto-y-roto.md) | si el nombre correcto es `Admin`, que sí existe y tiene diez personas dentro. Hoy la condición vale exactamente `is_superuser`, y de ella cuelgan seis rutas: las papeleras de grupos, profesores, perfiles y alumnos, y las cuatro masivas de cuentas. Es cambiar una línea, pero decide quién puede vaciar una papelera de 27 tablas |
| El interruptor con el que el colegio cierra el periodo a los profesores lo elige el cliente | [05 §27](05-codigo-muerto-y-roto.md) | cuál de las tres formas de cerrarlo. La 1 —exigir que `num_periodo` y `periodo_id` concuerden— cierra uniformes y las cuatro pantallas de Flutter en media hora y deja fuera `notas/update/{id}`; la 2 es el arreglo de verdad y son 26 consultas dentro del cálculo de notas; la 3 es de una línea y apaga cuatro pantallas de Flutter. **Es la única de esta lista que deja algo abierto ahora mismo** |
