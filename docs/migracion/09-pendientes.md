...
| **C** | ~~**La hora mal escrita en filas ya guardadas: ¿migración o nota?**~~ — **CERRADA COMO PREGUNTA el 6 sep 2026: medida en los DIECISIETE (13 con daño, 708 filas de `change_asked` y ~25.200 de `ausencias`) y la migración ya ESCRITA, probada y sin fundir en `fix/reparar-la-hora-y-uniformes`. Lo único que queda es si se corre, y va después del día 10 ([05 §250](05-codigo-muerto-y-roto.md)).** — y antes, **MEDIDA el 5 sep 2026, cuando la pregunta estaba mal planteada** ([05 §247](05-codigo-muerto-y-roto.md)). Estaba parada sobre *«el dato no distingue una fila mal escrita de una normal»*, que es cierto **de una fila suelta** y falso del conjunto: **hay firma** —en `hora:hora:minutos` los minutos se quedan con la hora, o sea `HOUR = MINUTE`, con ~1/60 de falsos positivos—. En `simonbolivar` la cumplen **0 de 85** filas de `deleted_at` y **0 de 7** de las ausencias del lector, con los cuatro controles en ruido. Falta lo único que contesta de verdad: **correr `tools/hora-escrita-dos-veces.php` en los diecisiete**, que es sólo lectura, no es una migración y cabe en una visita al servidor. **Las tres opciones y sus precios están en la §247**; ninguna toca la tanda del día 10 | `change_asked.deleted_at` (§121) · `created_at`/`updated_at` de las ausencias del lector (§123) |
