...
/*
 * Horario: las tres tablas de la §5.1, el puntero del año y el color del docente.
 *
 * ## Esto fueron DOS migraciones hasta el 4 sep 2026
 *
 * `2026_09_04_200000_tono_del_docente` se fusionó aquí **antes de desplegarse
 * nunca**, con el despliegue congelado por Joseth a la espera de la Play Store.
 * **Y esta fusión no ahorra ningún `ALTER`**, al contrario que la de nivelaciones:
 * `years` y `profesores` son tablas distintas, así que eran dos sentencias y
 * siguen siendo dos. Es recuento —un fichero menos que leer la noche del
 * despliegue— y se hace porque Joseth lo pidió sabiendo eso.
 *
 * **Lo que sí cuesta, dicho para que no se lea al revés dentro de dos meses:**
 * `tono` era la única de las ocho que se podía correr sola, y aquí hereda el
 * `->after('regla_nivelacion')` de `years.horario_version_id`. Eso **no** valida
 * la fila de `DESPLIEGUE.md` que decía que `tono` entraba con ese `AFTER`: esa
 * fila nació mal —la migración del tono no tenía ni un `after`, y `tono` va sobre
 * `profesores`, no sobre `years`— y está corregida allí con el `grep` delante.
 * *Una afirmación falsa no se corrige haciéndola cierta después.*
 *
 ...
 *
 * ═══════════════════════════════════════════════════════════════════════════
 * ## `profesores.tono` — el color del docente, decidido por Joseth el 4 sep 2026
 * ═══════════════════════════════════════════════════════════════════════════
 *
 ...
 *
 * El sitio de una columna dentro de `profesores` **no lo lee nadie**, así que no se
 * compra una dependencia de orden que no hace falta: va al final de la tabla. Es lo
 * contrario que `years.horario_version_id` aquí arriba, que sí se ata a
 * `regla_nivelacion` —de `2026_09_02_100000`— y por eso **no hay camino «sólo esta
 * migración»**: `ADD COLUMN … AFTER x` con una `x` que no existe no es aditivo, es un
 * error de columna desconocida. Las dos cosas conviven en este fichero desde la
 * fusión y **no son la misma**: quien reordene el `up()` tiene que saber cuál es cuál.
 *
 ...
 * **`tono` aparece en las seis sin que nadie la mande** — igual que le pasa a
 * `years.horario_version_id` con los tres `SELECT *` de `YearsController`, aquí
 * arriba. Vale `null` en todos, así que es lo más inofensivo que se le puede mandar a
 * los cuatro clientes, **pero es un campo nuevo y se manda dicho, no descubierto**.
 *
 ...
 *
 * El día que exista una ruta que la ponga, la longitud **se valida con 422 y no se deja
 * truncar**: en MySQL no estricto un valor más largo entra recortado y sin decir nada,
 * y un color truncado no da error — pinta otro color. Es la misma regla que
 * `horario_lecciones.pieza_id` (§5.1), donde el truncado fusionaba dos piezas en una.
 */
class HorarioVersiones extends Migration
{
    public function up()
    {
        ...
        // El color del docente. **SIN `after` a propósito** —ver la cabecera—: al
        // revés que la de `years` de aquí arriba, ésta no se ata a ninguna columna
        // de otra migración. Va al final de `profesores`.
     ...
     * subidas desde el despliegue y los colores repartidos a los docentes, y nada
     * más: **las siete columnas de día de
