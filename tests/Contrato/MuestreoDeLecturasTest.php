...
class MuestreoDeLecturasTest extends CasoDeContrato
{
    ...
    /**
     * Los huecos: las listas que salen vacías DENTRO de una respuesta que sí
     * trae datos.
     *
     * Son las que se cuelan. La respuesta pasa la comprobación de arriba porque
     * el resto viene lleno, y esa clave concreta se queda sin que nadie le mire
     * la forma —que es el fallo que el P0 cometió con `myimages` y la lista de
     * deudores, solo que anidado un nivel más adentro y por eso más difícil de
     * ver—.
     *
     * Se leen de los ficheros de snapshot en vez de volver a pedir las 42 rutas:
     * el snapshot ES la forma, así que la lista sale igual y sin peticiones. Y
     * de paso vigila lo que hay que vigilar, que es el momento en que alguien
     * regenera un snapshot y aparece un hueco nuevo.
     */
    public function test_los_huecos_del_seed_son_los_conocidos(): void
    {
        $conocidos = [
            // El año siguiente al del seed está borrado, así que no hay grupos
            // a los que promover. Ya anotado en docs/migracion/03-tests.md.
            'grupos-con-paises-tipos-next-year' => ['grupos'],

            // El tablero de inicio del profesor. Trae su horario y los cambios
            // que le han pedido, y el seed no tiene ni horario ni solicitudes.
            'ChangesAsked-to-me' => [
                'alumnos', 'horario_hoy', 'horario_manana',
                'intentos_fallidos', 'profesores', 'publicaciones.comentarios',
            ],

            // Nadie ha faltado en el grupo del seed. El camino dice dónde: la
            // planilla trae sus alumnos y sus periodos, y el hueco está en la
            // hoja del árbol.
            ...
            // Acudiente de LoginTest. `token` es el objeto vacío que quedó de
            // JWT, explicado en Services\ContextoDeUsuario.
            'auth-me' => ['perms', 'token'],
        ];

        $encontrados = [];

        foreach (glob(__DIR__.'/Snapshots/muestreo-*.json') as $fichero) {
            $nombre = basename($fichero, '.json');
            $huecos = $this->clavesVacias(json_decode(file_get_contents($fichero), true));

            if ($huecos !== []) {
                $encontrados[substr($nombre, strlen('muestreo-'))] = $huecos;
            }
        }
        ...
        $this->assertSame(
            $conocidos,
            $encontrados,
            "Cambió qué partes de la respuesta el seed deja sin comprobar.\n".
            "Si aparece un hueco nuevo, o el seed dejó de traer un dato, o la ruta\n".
            'dejó de devolverlo. Las dos cosas hay que mirarlas antes de anotarlo aquí.'
        );
    }
    ...
    private function clavesVacias($forma, string $camino = ''): array
    {
        ...
        foreach ($forma as $clave => $valor) {
            $bajo = $camino === '' ? (string) $clave : $camino.'.'.$clave;
            ...
            // Una lista con un elemento es cómo formaUnida() escribe la forma de
            // la lista entera. Bajar por su primer elemento sin numerarlo deja
            // el camino legible: `grupos.alumnos`, no `grupos.0.alumnos`.
            $vacias = array_merge(
                $vacias,
                $this->clavesVacias($valor, array_is_list($forma) ? $camino : $bajo)
            );
        }
