...
class NotasController extends Controller
{
    ...
    public function putDetailed()
    {
        ...
        foreach ($alumnos as $alumno) {
            if (BoletinIndependiente::aplica((int) $alumno->alumno_id, (int) $user->periodo_id)) {
                $independientes[] = [
        ...
        foreach ($alumnos as $alumno) {
            ...
            $notas = DB::select($cons, [':per_id' => $user->periodo_id, ':grupo_id' => $asignatura->grupo_id, ':alumno_id' => $alumno->alumno_id, ':asignatura_id' => $asignatura->asignatura_id,
                ':alcance' => BoletinIndependiente::alcance((int) $alumno->alumno_id, (int) $user->periodo_id)]);
    ...
    {
        $config = Nivelacion::reglaDelAnio($yearId);
    ...
    private function nivelarLaFila(object $fila, array $forma, object $user, Carbon $now, ?int $historialId): array
    {
        ...
        $aplicada = Nivelacion::aplicar($config['regla'], $original, $nivelacion, $config['nota_minima']);
        $nueva = $aplicada['nota'];
