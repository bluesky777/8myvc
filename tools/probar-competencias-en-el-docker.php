...
function pedirCompetencias(string $metodo, string $uri, array $cuerpo = [], ?string $token = null): array
{
    global $kernel;
...
function contarCompetencias(string $rotulo, array $r, $extra = null): void
{
...
// ── 1. Login de verdad ───────────────────────────────────────────────────────
$login = pedirCompetencias('POST', '/api/login/credentials', [
    'username' => 'administrador',
    'password' => 'patreongreat',
]);
$token = $login['cuerpo']['el_token'] ?? null;

contarCompetencias('POST login (administrador)', $login, $token === null ? 'SIN TOKEN' : 'token ok');
...
// ── 2. GET competencias ──────────────────────────────────────────────────────
$r = pedirCompetencias('GET', '/api/competencias', [], $token);
contarCompetencias('GET  competencias', $r, 'year_id='.($r['cuerpo']['year_id'] ?? '?')
    .' · filas='.count($r['cuerpo']['competencias'] ?? []));

// ── 3. GET competencias/catalogo-men ─────────────────────────────────────────
$r = pedirCompetencias('GET', "/api/competencias/catalogo-men?materia_id={$materia->id}&grado_id={$sexto->id}", [], $token);
contarCompetencias('GET  catalogo-men · '.$materia->materia, $r,
...
$r = pedirCompetencias('GET', "/api/competencias/catalogo-men?materia_id={$matematicas->id}&grado_id={$sexto->id}", [], $token);
contarCompetencias('GET  catalogo-men · '.$matematicas->materia, $r,
    'por_tipo='.json_encode($r['cuerpo']['por_tipo'] ?? null)
    .' · adoptables='.($r['cuerpo']['adoptables'] ?? '?'));

$r = pedirCompetencias('GET', "/api/competencias/catalogo-men?materia_id={$religion->id}&grado_id={$sexto->id}", [], $token);
contarCompetencias('GET  catalogo-men · '.$religion->materia, $r,
    'cubierta='.var_export($r['cuerpo']['cubierta'] ?? null, true)
    .' · '.($r['cuerpo']['emparejamiento']['materia'] ?? '?'));
echo '        motivo: '.($r['cuerpo']['motivo'] ?? 'null').PHP_EOL;

$r = pedirCompetencias('GET', "/api/competencias/catalogo-men?materia_id={$matematicas->id}&grado_id={$transicion->id}", [], $token);
contarCompetencias('GET  catalogo-men · Transición', $r, $r['cuerpo']['emparejamiento']['grado'] ?? '?');
echo '        motivo: '.($r['cuerpo']['motivo'] ?? 'null').PHP_EOL;

$r = pedirCompetencias('GET', '/api/competencias/catalogo-men', [], $token);
contarCompetencias('GET  catalogo-men SIN materia_id (tiene que ser 422)', $r);

// ── 4. PUT competencias/copiar con origen MEN ────────────────────────────────
$r = pedirCompetencias('PUT', '/api/competencias/copiar', [
    'destino' => ['materia_id' => $materia->id, 'grado_id' => $sexto->id],
    'origen' => ['tipo' => 'men'],
], $token);
contarCompetencias('PUT  copiar · origen men (1.ª vez)', $r, json_encode($r['cuerpo']));

$r = pedirCompetencias('PUT', '/api/competencias/copiar', [
    'destino' => ['materia_id' => $materia->id, 'grado_id' => $sexto->id],
    'origen' => ['tipo' => 'men'],
], $token);
contarCompetencias('PUT  copiar · origen men (2.ª vez)', $r,
    'copiadas='.($r['cuerpo']['copiadas'] ?? '?')
    .' · duplicadas='.($r['cuerpo']['saltadas_por_duplicado'] ?? '?'));

// ── 5. POST / PUT / DELETE / orden ───────────────────────────────────────────
$r = pedirCompetencias('POST', '/api/competencias', [
    'materia_id' => $materia->id,
    'grado_id' => $sexto->id,
    'definicion' => 'Competencia escrita a mano por la prueba del docker.',
], $token);
contarCompetencias('POST competencias', $r, 'id='.($r['cuerpo']['id'] ?? '?')
    .' · grupo='.($r['cuerpo']['grupo']['competencias'] ?? '?'));
$nueva = $r['cuerpo']['id'] ?? null;

$r = pedirCompetencias('PUT', '/api/competencias/'.$nueva, ['definicion' => 'Y ahora con otro texto.'], $token);
contarCompetencias('PUT  competencias/{id}', $r, '«'.($r['cuerpo']['definicion'] ?? '?').'»');

$grupo = pedirCompetencias('GET', "/api/competencias?materia_id={$materia->id}&grado_id={$sexto->id}", [], $token);
$ids = array_column($grupo['cuerpo']['competencias'] ?? [], 'id');
$r = pedirCompetencias('PUT', '/api/competencias/orden', [
    'materia_id' => $materia->id, 'grado_id' => $sexto->id, 'orden' => array_reverse($ids),
], $token);
contarCompetencias('PUT  competencias/orden', $r, 'reordenadas='.($r['cuerpo']['reordenadas'] ?? '?'));

$r = pedirCompetencias('PUT', '/api/competencias/orden', [
    'materia_id' => $materia->id, 'grado_id' => $sexto->id, 'orden' => [$nueva],
], $token);
contarCompetencias('PUT  orden con lista parcial (tiene que ser 422)', $r);

$r = pedirCompetencias('DELETE', '/api/competencias/'.$nueva, [], $token);
contarCompetencias('DEL  competencias/{id}', $r, 'id='.($r['cuerpo']['id'] ?? '?'));
