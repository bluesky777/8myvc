...
| **R** · **EN VUELO** | **`profesores.tono` se reparte solo a TRECE respuestas vivas.** La lista canónica **no se repite aquí**: vive en `docs/AVISO-A-LOS-CLIENTES-tono.md`, que es lo que se le entregó al front. Once devuelven el modelo Eloquent entero —las cinco de `ProfesoresController` (`postStore`, `putUpdate`, `deleteDestroy`, `deleteForcedelete`, `putRestore`), `GruposController::getShow` y su gemela `PerfilesController::getShow`, `PerfilesController::putUpdate` y `::putCambiarimgunprofe`, e `ImagesUsuariosController::putCambiarFotoUnUsuario` y `::putCambiarFirmaUnProfe`— y **dos son SQL crudo**: `PUT profesores/listado` y `PUT participantes/profesores`. Vale `null` en todas | **POR AVISAR** con la rama de la cuarta ruta — campo nuevo. **Aquí decía SEIS y era un subconjunto**: se barrieron `ProfesoresController` y `GruposController`, que es el alcance que traía el encargo, y no el resto de la API. Corregido a trece el 4 sep 2026 con el barrido de `8myvc-ff`, verificado sitio por sitio |
...
> **La fila R se escribió mal dos veces seguidas, y la segunda es la que enseña algo.** Nació
> diciendo **seis** —las respuestas Eloquent de `ProfesoresController` y `GruposController`—, que es
> el alcance que traía el encargo y no el de la pregunta. Al revisarla se ensanchó **un eje**: se
> barrió el **SQL crudo**, salió que `DocentesExport` trae la fila entera y filtra en la vista, y se
> escribió *«seis sigue siendo seis, y ahora dice por dónde se buscó»*. **Y el otro eje seguía sin
> tocar**: qué controladores. Son **trece**, y las siete que faltaban están en `PerfilesController`,
> `ImagesUsuariosController` y dos consultas crudas —`PUT profesores/listado` y
> `PUT participantes/profesores`—, todas fuera del alcance mirado. Lo levantó `8myvc-ff` con
> `tools/filas-enteras-al-cliente.php` ya arreglada; verificadas aquí una a una antes de aceptarlas.
>
> *Ensanchar la búsqueda por un eje y dar el número por confirmado es peor que no haberla ensanchado:
> el «ahora dice por dónde se buscó» es exactamente lo que hace que nadie vuelva a mirar.* Es la misma
> forma que esta tabla ya tenía catalogada tres veces en la fila de `horario_versiones` —mirar el
> módulo en vez de mirar quién lee la columna—, cometida por quien la escribió.
