...
class BolfinalesController extends Controller {
	...
	public function detailedNotasGrupo($grupo_id, $user, $requested_alumnos='')
	{
		...
		// Aquí `$year->periodos` es una **Collection** (`periodosDelAnio()` hace
		// `->get()->map()`), no un array como en el gemelo de `Informes`: `array_map`
		// sobre ella sería un TypeError, no una lista vacía.
		BoletinIndependiente::ponerPuestos(
			$alumnos,
			$year->periodos->map(static fn ($periodo) => (int) $periodo->id)->all(),
			(int) $user->year_id
		);
